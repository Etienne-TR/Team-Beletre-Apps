<?php
/**
 * API de vue individuelle des responsabilités
 * 
 * Fournit les endpoints pour consulter les responsabilités
 * et tâches d'un utilisateur spécifique à une date donnée.
 */
...
class IndividualResponsibilitiesAPI extends BaseAPI {
    ...
    protected function handleRequest() {
        ...
        switch ($action) {
            case 'get_workers':
                $this->getWorkersWithContracts();
                break;
            case 'get_person_activities':
                $this->getPersonActivities();
                break;
            case 'get_person_tasks':
                $this->getPersonTasks();
                break;
            case 'get_person_other_activities':
                $this->getPersonOtherActivities();
                break;
            default:
    ...
    private function getWorkersWithContracts() {
        $date = $_GET['date'] ?? date('Y-m-d');
        
        // Log pour débogage
        error_log("=== DÉBUT getWorkersWithContracts ===");
        ...
        error_log("=== FIN getWorkersWithContracts ===");
        
        $this->sendSuccess(['workers' => $workers]);
    }
    
    private function getPersonActivities() {
        ...
        // Get activities where the user is directly responsible
        $sql = "
        ...
        foreach ($activities as $activity) {
            ...
            // Get tasks assigned to the user for this activity
            $sql_tasks = "
                SELECT 
                    t.entry as task_id,
                    t.name as task_name,
        ...
        $this->sendSuccess(['activities' => $result]);
    }
    
    private function getPersonTasks() {
        $userId = $_GET['user_id'] ?? null;
        $date = $_GET['date'] ?? date('Y-m-d');
    ...
    private function getPersonOtherActivities() {
        $userId = $_GET['user_id'] ?? null;
        $date = $_GET['date'] ?? date('Y-m-d');
        ...
        // Get activities where the user is NOT responsible but has tasks assigned
        $sql = "
            SELECT DISTINCT
                a.entry as activity_id, 
                a.name as activity_name, 
        ...
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$userId, $date, $date, $userId, $date, $date]);
        $activities = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
new IndividualResponsibilitiesAPI(); 